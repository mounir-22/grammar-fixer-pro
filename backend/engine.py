...
class LLMEngine:
    ...
    def _setup_llm(self):
        ...
        self.use_llm = replicate is not None and self.api_key
        if self.use_llm:
...
Output:"""
        ...
        try:
            ...
            try:
                ...
                print(f"JSON parse error for naturalness. Attempting fallback extraction...")
                print(f"Raw output (first 200 chars): {output[:200]}")
                
                # Try fallback text extraction
                enhanced_text = self._extract_text_fallback(output, text)
                if enhanced_text != text:
                    return {
                        "text": enhanced_text,
                        "changes": [{"original": "(parsing failed)", "suggestion": "text enhanced", "reason": "JSON parsing failed, used fallback"}],
                        "enhancement_type": "naturalness"
                    }
                else:
                    raise RuntimeError(f"JSON parsing failed and fallback unsuccessful: {je}")
...
Output:"""
        ...
        try:
            ...
            try:
                ...
                print(f"JSON parse error for formality. Attempting fallback extraction...")
                print(f"Raw output (first 200 chars): {output[:200]}")
    ...
    def _extract_text_fallback(self, output, original_text):
        """Fallback method to extract enhanced text when JSON parsing fails"""
        try:
            ...
            # Look for patterns like "Output: text" or "Result: text"
            patterns = [
                r'(?:Output|Result|Enhanced text|Formal text|Natural text):\s*["\']?(.*?)["\']?(?:\n|$)',
                r'"text":\s*"(.*?)"',
                r'text:\s*["\']?(.*?)["\']?(?:\n|,|\})',
            ]
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    if text and len(text) > len(original_text) * 0.5:  # Sanity check
                        return text
            
            # If all else fails, try to find text that looks like enhanced version
            lines = output.split('\n')
            for line in lines:
                line = line.strip()
                if (len(line) > len(original_text) * 0.5 and 
                    len(line) < len(original_text) * 2 and
                    line != original_text):
                    return line
